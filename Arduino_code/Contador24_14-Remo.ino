/* ==========================================================================
   PROYECTO: CONTADOR ESCLAVO REMO (Sincronizado con Rómulo)
   ========================================================================== */
...
// Pines correctos (Físicamente ya los dejaste bien)
...
void procesarComando(byte cmd) {
  ...
  else if (cmd >= 1 && cmd <= 99) { valorRestanteSec = cmd; valorInicialSec = cmd; countdownActivo = false; renderValorActual(cmd); }
}

void setup() {
  Serial.begin(9600);
  BTEsclavo.begin(115200);

  Timer1.initialize(4000);
  Timer1.attachInterrupt(scanDMD);
  
  renderValorActual(valorRestanteSec);
  Serial.println("REMO ESCLAVO listo. Escuchando a Romulo...");
}

void loop() {
  if (BTEsclavo.available()) {
    byte cmd = BTEsclavo.read(); 
    
    // FILTRO DE RUIDO Y ESTÁTICA
    if (cmd == 0 || cmd == 255) return; 

    Serial.print("Romulo ordeno el Byte: "); Serial.println(cmd);
    procesarComando(cmd); 
  }
  ...
  if (countdownActivo && valorRestanteSec >= 0) {
    ...
    if (now - lastTickMs >= TICK_SEC_MS) {
      ...
      if (valorRestanteSec < 0) {
        valorRestanteSec = 0; countdownActivo = false;
      }
      renderValorActual(valorRestanteSec);
    }
  }
